...
impl TemperatureLevel {
    ...
    pub fn force_active_if_pending(&mut self, time_s: u64) {
        if self.state == ProfileState::Pending {
            self.state = ProfileState::Active;
            self.start_time_s = time_s;
        }
...
impl TemperatureProfile {
    ...
    pub fn start(&mut self, time_s: u64, temperature_c: f32) -> f32 {
        if self.state != ProfileState::Inactive {
    ...
    pub fn start_pending_level(&mut self, time_s: u64, temperature_c: f32) {
    ...
    pub fn terminate_current_level(&mut self, time_s: u64, temperature_c: f32) {
        if self.active_level >= 0 && self.state != ProfileState::Terminated {
            let level = &mut self.levels[self.active_level as usize];
    ...
    pub fn update(&mut self, time_s: u64, temperature_c: f32) -> f32 {
        if self.active_level >= 0 && self.state != ProfileState::Terminated {
